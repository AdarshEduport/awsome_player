...
/// See also:
///
///   * [InteractiveViewerWidget.builder], whose builder is of this type.
///   * [WidgetBuilder], which is similar, but takes no viewport.
typedef InteractiveViewerWidgetWidgetBuilder = Widget Function(
...
///
/// By default, InteractiveViewerWidget clips its child using [Clip.hardEdge].
/// To prevent this behavior, consider setting [clipBehavior] to [Clip.none].
/// When [clipBehavior] is [Clip.none], InteractiveViewerWidget may draw outside of
...
/// To prevent dead areas where InteractiveViewerWidget does not receive gestures,
/// don't set [clipBehavior] or be sure that the InteractiveViewerWidget widget is the
...
///     which includes the use of InteractiveViewerWidget.
///   * The [flutter-go demo](https://github.com/justinmc/flutter-go), which includes robust positioning of an InteractiveViewerWidget child
///     that works for all screen sizes and child sizes.
///   * The [Lazy Flutter Performance Session](https://www.youtube.com/watch?v=qax_nOpgz7E), which includes the use of an InteractiveViewerWidget to
...
class InteractiveViewerWidget extends StatefulWidget {
  /// Create an InteractiveViewerWidget.
  ...
  /// Creates an InteractiveViewerWidget for a child that is created on demand.
  ///
  ...
  /// large child.
  InteractiveViewerWidget.builder({
    super.key,
    this.clipBehavior = Clip.hardEdge,
    this.panAxis = PanAxis.free,
    this.boundaryMargin = EdgeInsets.zero,
    ...
    this.trackpadScrollCausesScale = false,
    this.overlay,
    required InteractiveViewerWidgetWidgetBuilder this.builder,
  })  : assert(minScale > 0),
        assert(interactionEndFrictionCoefficient > 0),
        assert(minScale.isFinite),
        assert(maxScale > 0),
  ...
  /// If set to [Clip.none], the child may extend beyond the size of the InteractiveViewerWidget,
  /// but it will not receive gestures in these areas.
  /// Be sure that the InteractiveViewerWidget is the desired size when using [Clip.none].
  ///
  ...
  ///
  /// Passed with the [InteractiveViewerWidget.builder] constructor. Otherwise, the
  ...
  final InteractiveViewerWidgetWidgetBuilder? builder;

  /// The child [Widget] that is transformed by InteractiveViewerWidget.
  ///
  /// If the [InteractiveViewerWidget.builder] constructor is used, then this will be
  ...
  /// is often useful when a child should be bigger than the InteractiveViewerWidget.
  ///
  ...
  ///
  /// {@template flutter.widgets.InteractiveViewerWidget.onInteractionEnd}
  ...
  ///
  /// A [GestureDetector] wrapping the InteractiveViewerWidget will not respond to
  ...
  /// listeners are notified. If the value is set, InteractiveViewerWidget will update
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      InteractiveViewerWidget.getNearestPointOnLine(
...
class _InteractiveViewerWidgetState extends State<InteractiveViewerWidget>
  ...
  Rect get _boundaryRect {
    ...
    assert(!boundaryRect.isEmpty,
        "InteractiveViewerWidget's child must have nonzero dimensions.");
  ...
  @override
  void didUpdateWidget(InteractiveViewerWidget oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.child != null) {
      child = _InteractiveViewerWidgetBuilt(
        childKey: _childKey,
      ...
      // When using InteractiveViewerWidget.builder, then constrained is false and the
      ...
      child = LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return _InteractiveViewerWidgetBuilt(
            childKey: _childKey,
            clipBehavior: widget.clipBehavior,
            constrained: widget.constrained,
            alignment: widget.alignment,
...
// InteractiveViewerWidget's depending on if it's using a builder or a child.
class _InteractiveViewerWidgetBuilt extends StatelessWidget {
  const _InteractiveViewerWidgetBuilt({
    required this.child,
...
/// See also:
///
///  * [InteractiveViewerWidget.transformationController] for detailed documentation
///    on how to use TransformationController with [InteractiveViewerWidget].
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return InteractiveViewerWidget.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        InteractiveViewerWidget.getNearestPointInside(point, boundary);
    final Offset excess =
...
/// This enum is used to specify the behavior of the [InteractiveViewerWidget] when
